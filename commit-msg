...
validate_commit_message() {
  ...
  ASPELL=$(which aspell)
  if [ $? -ne 0 ]; then
    echo "GNU ASPELL is not installed - unable to check spelling"
  else
    LINE_NUMBER=1
    MISS_SPELLED_WORDS=`echo "$COMMIT_MSG_LINES[LINE_NUMBER]"` | $ASPELL --lang=en --list
    if [ -n "$MISS_SPELLED_WORDS"]; then
      add_warning LINE_NUMBER "Possible misspelled word(s): $MISS_SPELLED_WORDS"
    fi
  fi
